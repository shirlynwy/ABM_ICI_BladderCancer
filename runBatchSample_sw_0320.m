function runBatchSample_sw_0320(I)
% rng(I) % worked on hpc

%testing below
% tempstr = string(datetime("now","Format","MMddHHmmss"));
tempstr = string(datetime("now","Format","yyMMdd"));
...
M.save_pars.batch_name = strcat('batch_', string(datetime("now","Format","yyMMddHH")));
...
M.immune_pars.prolif_rate = 0; % immune proliferation rate per day
M.immune_pars.immune_recruit_rate2 = linspace(2,20,10)'; % immune cells recruited per day
